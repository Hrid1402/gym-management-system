...
export const ClientMembershipPage: React.FC = () => {
  ...
  const loadPlans = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Failed to load plans');
    } finally {
      setLoadingPlans(false);
    }
  ...
  if (loadingPlans) return <LoadingState message="Loading membership details..." />;

  if (activeMem) {
    return (
      <div>
        <PageHeader title="Acquire Membership" subtitle="Membership acquisition is restricted" />
          ...
          <Card>
            <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
              ...
              <p style={{ fontSize: '0.875rem', color: 'var(--color-neutral-600)', marginTop: '0.5rem', marginBottom: '1.5rem' }}>
                You already have an active gym membership (<strong>{activeMem.planName || 'Current Plan'}</strong>). The system does not allow purchasing multiple concurrent memberships. To purchase a new plan, please cancel your existing membership first.
              </p>
  ...
  const handleConfirm = async (e: React.FormEvent) => {
    ...
    try {
      // WEB MEMBERSHIP REGISTRATION: POST /api/memberships/web-register
      const mem = await membershipService.webRegisterMembership({
        planId: selectedPlanId,
      });
      setCreatedMembership(mem);
    } catch (err: any) {
      setErrorMsg(err.message || 'Failed to register membership');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (createdMembership) {
    return (
      <div>
        <PageHeader title="Membership Confirmation" subtitle="Your membership registration was successful" />
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          <Card>
            <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
              ...
              <p style={{ fontSize: '0.875rem', color: 'var(--color-neutral-500)', marginTop: '0.25rem' }}>
                Your plan enrollment is confirmed.
              </p>
            </div>
            ...
            <div style={{ backgroundColor: 'var(--color-neutral-50)', padding: '1.25rem', borderRadius: 'var(--radius-md)', marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                <span style={{ fontSize: '0.875rem', color: 'var(--color-neutral-600)' }}>Selected Plan:</span>
                <strong style={{ fontSize: '1rem', color: 'var(--color-neutral-900)' }}>{selectedPlan?.name}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                <span style={{ fontSize: '0.875rem', color: 'var(--color-neutral-600)' }}>Status:</span>
                <StatusBadge status={createdMembership.status} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                <span style={{ fontSize: '0.875rem', color: 'var(--color-neutral-600)' }}>Start Date:</span>
                <strong style={{ fontSize: '0.875rem', color: 'var(--color-neutral-800)' }}>{createdMembership.startDate}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '0.875rem', color: 'var(--color-neutral-600)' }}>End Date:</span>
                <strong style={{ fontSize: '0.875rem', color: 'var(--color-neutral-800)' }}>{createdMembership.endDate}</strong>
              </div>
            </div>
  ...
  return (
    <div>
      ...
      <PageHeader title="Acquire Membership" subtitle="Select a plan to enroll in your gym membership" />

      <div style={{ maxWidth: '600px' }}>
        <Card title="Membership Registration">
            ...
            {selectedPlan && (
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem', marginBottom: '0.25rem' }}>
                  <span style={{ color: 'var(--color-neutral-600)' }}>Plan Name:</span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem', marginBottom: '0.25rem' }}>
                  <span style={{ color: 'var(--color-neutral-600)' }}>Total Price:</span>
